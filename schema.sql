...
-- Pronostic PAR MATCH (comme tn_bracket_predictions, migration 0015), mais
-- restreint à UN SEUL TOUR affiché/édité à la fois dans l'écran — chaque
-- tour reste indépendant, aucun n'écrase ni n'accumule les autres. Barème
-- (2^(tour−1)) et score calculés à la volée par lib/bracketSim.ts
-- (`scoreDuStock`), jamais stockés. Même convention de stock que tn_picks
-- (participant_id null = moi).
--
-- Remplace tn_bracket_anchors (migration 0016, ancre unique par stock,
-- sans notion de match/position).
-- ---------------------------------------------------------------------
...
-- (lib/fantasy.ts), que figer rendrait périmée au moindre changement de
-- barème ou de paliers.
-- ---------------------------------------------------------------------
...
begin
  foreach t in array array['tn_players','tn_tournaments','tn_matches',
                           'tn_participants',
                           'tn_picks','tn_bracket_round_picks','tn_simulated_picks',
...
revoke all on table
  ...
  tn_bracket_round_picks, tn_simulated_picks, tn_projections, tn_fantasy, tn_fantasy_historique
